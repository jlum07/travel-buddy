...
const cacheExpiryTimeMins = 60 * 10; // 10 hours
...
module.exports = knex => {
  router.get("/autocorrect/:name", (req, res) => {
    ...
    let url = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${
      req.params.name
    }&types=(cities)&key=${process.env.GOOGLE_MAPS_API_KEY || API_KEY.API_KEY}`;
    fetch(url)
      .then(res => res.json())
      .then(json => {
        if (json.status === "REQUEST_DENIED") {
          console.log(
            "Google Autocorrect API request DENIED (API KEY INVALID??)"
          );
    ...
    knex
      ...
      .then(async DBsearchResponse => {
        if (DBsearchResponse.length > 0) {
          ...
          if (ageOfData_mins < cacheExpiryTimeMins) {
            console.log(
              `Data for ${req.params.city} is ${Number(ageOfData_mins).toFixed(
                2
              )} minutes old --> STILL REVELENT (<${cacheExpiryTimeMins} mins old)`
            );
            ...
            console.log(
              `Data for ${req.params.city} is ${Number(ageOfData_mins).toFixed(
                2
              )} minutes old --> EXPIRED (>${cacheExpiryTimeMins} mins old)...`
            );
            ...
            knex("city_data_cache")
              ...
              .catch(error => {
                console.log(
                  `Failed to update ${req.params.city} in DB: ${error}`
                );
          ...
          // City is NOT IN DB:
          console.log(
            `${req.params.city} not in DB...collecting city data ...`
          );
      ...
      .catch(error => {
        console.log(
          "Error while trying to search for city in city_data_cache table in DB:",
          error
        );
      });
  });
